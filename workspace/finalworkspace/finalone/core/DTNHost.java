...
public class DTNHost implements Comparable<DTNHost> , Iterable<Connection>{
	...
	private HashMap<DTNHost ,HashMap<Integer,Integer>>history_utility;////历史效用表(包括仿真开始到当前时间间隔和节点相遇次数)
	private HashMap<DTNHost ,HashMap<String,Double>> fin_utility;//fin效用表(包括投递概率、位置、速度等)
	...
	//更新效用表(set)
	public void setFinUtility(DTNHost host,HashMap<String,Double> dtnhash) {
		this.fin_utility.put(host, dtnhash);
	}
	//获取节点速度
	public double getSpeed() {
		return this.speed;
	}
	public double getlastAgeUpdate() {
		return this.lastAgeUpdate;
	}
	public void setlastAgeUpdate(double time) {
		this.lastAgeUpdate=time;
	}
