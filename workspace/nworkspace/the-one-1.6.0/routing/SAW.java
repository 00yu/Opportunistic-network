 ...
 *
 */
public class SAW extends ActiveRouter {
	...
	public static final String SAW_NS = "SAW";
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		Map<DTNHost, Double> othersPreds = 
			((SAW)otherRouter).getDeliveryPreds();
	...
	public Message messageTransferred(String id, DTNHost from) {//接收方
		...
		SAW othRouter = (SAW)from.getRouter();
		//此时是发送端没有分配前的总副本数
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		Collection<Message> msgCollection = getMessageCollection();
			...
			for (Connection con : getConnections()) {
				...
				SAW othRouter = (SAW)other.getRouter();
				...
				for (Message m : msgCollection) {
					DTNHost otherHost=con.getOtherNode(getHost());
					SAW otherRouter = (SAW)otherHost.getRouter();
