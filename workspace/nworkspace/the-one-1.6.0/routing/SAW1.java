 ...
 *
 */
public class SAW1 extends ActiveRouter {
	...
	public static final String SAW_NS = "SAW1";
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		Map<DTNHost, Double> othersPreds = 
			((SAW1)otherRouter).getDeliveryPreds();
	...
	public Message messageTransferred(String id, DTNHost from) {//接收方
		...
		SAW1 othRouter = (SAW1)from.getRouter();
		//此时是发送端没有分配前的总副本数
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		Collection<Message> msgCollection = getMessageCollection();
			...
			for (Connection con : getConnections()) {
				...
				SAW1 othRouter = (SAW1)other.getRouter();
				...
				for (Message m : msgCollection) {
					DTNHost otherHost=con.getOtherNode(getHost());
					SAW1 otherRouter = (SAW1)otherHost.getRouter();
					...
					if(copies1>1) {
						if(p1+p2>0) {
								...
								if(!otherRouter.hasMessage(m.getId())) {
									int copies2new=(int)Math.floor(p2/(p1+p2)*copies1);
									int copies1new=copies1-copies2new;
									if(copies2new>0) {
	...
	protected void transferDone(Connection con) {
		...
		if (isBinary) { 
			nrofCopies =nrofCopies-(int)Math.floor(p2/(p1+p2)*nrofCopies);
		}
		if(nrofCopies==0) {
	...
	@Override
	public SAW1 replicate() {
		return new SAW1(this);
	}
}
