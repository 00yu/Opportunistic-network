 ...
 *
 */
public class newSAW extends ActiveRouter {
	...
	public static final String SAW_NS = "newSAW";
	...
	public newSAW(Settings s) {
		...
		thread=sawSettings.getDouble("thread");
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		Map<DTNHost, Double> othersPreds = 
			((newSAW)otherRouter).getDeliveryPreds();
	...
	public Message messageTransferred(String id, DTNHost from) {//接收方
		...
		newSAW othRouter = (newSAW)from.getRouter();
		//此时是发送端没有分配前的总副本数
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		Collection<Message> msgCollection = getMessageCollection();
			...
			for (Connection con : getConnections()) {
				...
				newSAW othRouter = (newSAW)other.getRouter();
				...
				for (Message m : msgCollection) {
					DTNHost otherHost=con.getOtherNode(getHost());
					newSAW otherRouter = (newSAW)otherHost.getRouter();
	...
	protected void transferDone(Connection con) {
		...
		if (isBinary) { 
			if(Math.abs(p1-p2)>thread) {
				nrofCopies =(int)(p2/(p1+p2)*nrofCopies);
			}
			else {
	...
	@Override
	public newSAW replicate() {
		return new newSAW(this);
	}
}
