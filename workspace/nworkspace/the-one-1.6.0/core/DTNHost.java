...
public class DTNHost implements Comparable<DTNHost> {
	...
	/**节点分离时间=当前连接开始时间-上次连接断开时间*/
	private Map<DTNHost,Double> separationDuration;
	private Map<DTNHost,Double> propertion;
	/**节点连接开始时间*/
	private Map<DTNHost, Double> startTime;
	private Map<DTNHost,Double> endTime;
	private Map<DTNHost,Integer> freq;
	...
	public DTNHost(List<MessageListener> msgLs,
		...
		this.separationDuration=new HashMap<DTNHost,Double>();
		this.propertion=new HashMap<DTNHost,Double>();
	...
	public double getstartTime(DTNHost host) {
		if(startTime.containsKey(host)) {
			return startTime.get(host);
		}
		else {
			return 0;
		}
	}
	public void setstartTime(DTNHost host,double start) {
		startTime.put(host, start);
	}
	public double getendTime(DTNHost host) {
		if(endTime.containsKey(host)) {
			return endTime.get(host);
		}
		else {
			return 0;
		}
	}
	public void setendTime(DTNHost host,double end) {
		endTime.put(host, end);
	}
	public double getpropertion(DTNHost host) {
		if(propertion.containsKey(host)) {
			return propertion.get(host);
		}
		else {
			return 0;
		}
	}
	public void setpropertion(DTNHost host,double prop) {
		propertion.put(host, prop);
	}
	public double getseparationDuration(DTNHost host) {
		if(separationDuration.containsKey(host)) {
			return separationDuration.get(host);
		}
		else {
			return 0;
		}
	}
	public void setseparationDuration(DTNHost host,double duration) {
		separationDuration.put(host, duration);
	}
	public int getfreq(DTNHost host) {
		if(freq.containsKey(host)) {
			return freq.get(host);
		}
		else {
			return 0;
		}
	}
	public void setfreq(DTNHost host,int f) {
		freq.put(host, f);
	}
}
