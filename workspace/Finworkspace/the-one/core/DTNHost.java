...
public class DTNHost implements Comparable<DTNHost> {
	...
	/*添加代码*/
	private List<DTNHost> E= new ArrayList<DTNHost>();	//相遇节点集合
	private List<DTNHost> EG= new ArrayList<DTNHost>();	//节点的社交群
	private HashMap<Message,Integer> fin_utility;//fin效用表(包括消息和对应消息副本数)
	private HashMap<Message,Integer> last_utility;//fin效用表(包括消息和对应消息副本数)
	...
	public DTNHost(List<MessageListener> msgLs,
		...
		//添加代码
		this.fin_utility=new HashMap<Message,Integer>();
		this.last_utility=new HashMap<Message,Integer>();
	...
	//获取相遇节点集合
	public List<DTNHost> getE(){
		return this.E;
	}
	public void setE(List<DTNHost> e) {
		this.E=e;
	}
	//获取节点社交群
	public List<DTNHost> getEG() {
		return this.EG;
	}
	public void setEG(List<DTNHost> eG) {
		this.EG = eG;
	}
	//当前节点和消息目的节点的相似度
	public double getSimilar(DTNHost other) {//传入消息目的节点
		double similara;
		List<DTNHost> lista=this.getE();//获取当前节点相遇节点集合
		List<DTNHost> listb=other.getE();//获取消息目的节点相遇节点集合
		List<DTNHost> listc=new ArrayList<DTNHost>();
		if(lista==null||listb==null) 
			similara=0.0;
		else {
			listc.addAll(lista);
			listc.retainAll(listb);//从该列表(listc)中删除指定集合(listb)中未包含的所有元素。 
			similara=listc.size();//lista和listb中相同元素个数
		}
		return similara;
	}
	//获得效用表(get)
		public HashMap<Message,Integer> getFinUtility() {
			return this.fin_utility;
		}
		//更新效用表(set)
		public void setFinUtility(Message m, int copies) {
			this.fin_utility.put(m, copies);
		}
		//获得效用表(get)
				public HashMap<Message,Integer> getLastUtility() {
					return this.last_utility;
				}
				//更新效用表(set)
				public void setLastUtility(Message m, int copies) {
					this.last_utility.put(m, copies);
				}
}
