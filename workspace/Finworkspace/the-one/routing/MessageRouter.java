...
public abstract class MessageRouter {
	...
	//自己添加代码(按最近接收消息、跳数最少的顺序排序)
	@SuppressWarnings(value = "unchecked") 
	protected List sortByQueueNewMode(List list) {
		Collections.sort(list, 
				new Comparator() {
			/** Compares two tuples by their messages' receiving time */
			public int compare(Object o1, Object o2) {
				...
				diff = m1.getReceiveTime()/m1.getPath().size() - m2.getReceiveTime()/m2.getPath().size();
				if (diff == 0) {
					return 0;
				}
				return (diff >0 ? -1 : 1);//从大到小拍（先发最近接收，跳数少的）
			}
		});
