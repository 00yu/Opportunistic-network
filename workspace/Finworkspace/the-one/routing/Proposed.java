...
public class Proposed extends ActiveRouter {
	...
	public static final double α=0.5;
	/**
	...
	public static final String SAW_NS = "Proposed";
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		Collection<Message> msgCollection = getMessageCollection();
			...
			for (Connection con : getConnections()) {
				...
				Proposed othRouter = (Proposed)other.getRouter();
				...
				for (Message m : msgCollection) {
					DTNHost otherHost=con.getOtherNode(getHost());
					Proposed otherRouter = (Proposed)otherHost.getRouter();
					double S=getHost().getSimilar(otherHost);
					double E=Math.max(getHost().getE().size(), otherHost.getE().size());
					double p1=getPredFor(m.getTo())+α*(1-S/E);
					double p2=otherRouter.getPredFor(m.getTo())+α*(1-S/E);
	...
	protected void transferDone(Connection con) {
		...
		double S=getHost().getSimilar(otherHost);
		double E=Math.max(getHost().getE().size(), otherHost.getE().size());
		double p1=getPredFor(msg.getTo())+α*(1-S/E);
		double p2=otherRouter.getPredFor(msg.getTo())+α*(1-S/E);
		if (isBinary) { 
