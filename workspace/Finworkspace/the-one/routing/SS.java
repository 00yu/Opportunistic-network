 ...
 *
 */
public class SS extends ActiveRouter {
	...
	public static final String SAW_NS = "SS";
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		Map<DTNHost, Double> othersPreds = 
			((SS)otherRouter).getDeliveryPreds();
		...
		protected List<Connection>  getConnectionByStrategy(Message msg, List<Connection> connections) {
			...
			for (Connection con : connections) {
				...
				if(getHost().getEG().contains(msg.getTo())){//当前节点和目的节点在一个社交群(Prophet)
					SS othRouter = (SS)other.getRouter();
					//目的节点和相遇节点也在一个社区，若相遇节点到目的节点的可能性越大，则转发
					if(other.getEG().contains(msg.getTo())&&(othRouter.getPredFor(msg.getTo()) > ((SS) getHost().getRouter()).getPredFor(msg.getTo()))){
