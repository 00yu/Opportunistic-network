...
public abstract class MessageRouter {
	...
	/** Setting value for my mode */
	public static final int Q_MODE_MY_NEW = 3;
	...
	private ConcurrentHashMap<String, Message> messages; //并发容器，采用了锁分离策略
	 ...
	 */
	public void update(){//事件处理函数取决于具体事件
	...
	public int receiveMessage(Message m, DTNHost from) {
		...
		this.putToIncomingBuffer(newMessage, from);//加入该节点的缓存		
		newMessage.addNodeOnPath(this.host);//加入消息的经过路径
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		if (!isFinalRecipient && outgoing!=null) {
			...
			// -> put to buffer
			//保存到内存
			//System.out.println("自定义副本数"+aMessage.getProperty("spray_times"));
			//System.out.println("父副本数"+aMessage.getProperty("SprayAndWaitRouter.copies"));
			addToMessages(aMessage, false);
		}
		else if (isFirstDelivery) {//是最终的且第一次投递
			this.deliveredMessages.put(id, aMessage);
//			this.getHost().addAck(id);
		}
	...
	/**
	 * 将消息传递到传入消息缓冲区，具有相同ID的两条消息由主机区分。
	...
	public void deleteMessage(String id, boolean drop) {		
		...
		for (MessageListener ml : this.mListeners) {//通知监听器更新report内容
	...
	public void drop_pob(boolean drop)
	{
		if(drop== true)
		{
			this.host.updatedrop();
			
		}
	}
	//返回投递成功的消息列表
	public HashMap<String, Message> get_delivermsg()
	{
