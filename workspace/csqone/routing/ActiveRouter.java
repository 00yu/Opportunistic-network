...
public abstract class ActiveRouter extends MessageRouter {
	...
	protected Message getOldestMessage(boolean excludeMsgBeingSent) {
		...
		for (Message m : messages) {
			...
			//接收时间小，转发次数多，概率小的删除
			int cishu=m.getPath().size();
			if (oldest == null ) {
				oldest = m;
				
				min=( m.getReceiveTime())/cishu;
			}
			else if (min> ( m.getReceiveTime())/cishu) {
				oldest = m;
				min=( m.getReceiveTime())/cishu;
			}
		}
	...
	{//根据效用复制
		...
		MyRouter othRouter = (MyRouter)otherNode.getRouter();
		for (Message m : messages) {		
					
					if(othRouter.getPredFor(m.getTo()) > ((MyRouter) getHost().getRouter()).getPredFor(m.getTo())){
...
//						System.out.println("本大不转发");
	...
	protected List<Connection>  getConnectionByStrategyWait(Message msg, List<Connection> connections) {
		List<Connection> newConnections=new ArrayList<Connection>();

		for (Connection con : connections) {
			DTNHost other= con.getOtherNode(getHost());
			
			//Myroutingspray othRouter = (Myroutingspray)other.getRouter();
			MyRouter othRouter = (MyRouter)other.getRouter();

				//目的节点和相遇节点也在一个社区，若相遇节点到目的节点的可能性越大，则转发
				if(othRouter.getPredFor(msg.getTo()) > ((MyRouter) getHost().getRouter()).getPredFor(msg.getTo())){
	...
	protected List<Connection>  getConnectionByStrategy(Message msg, List<Connection> connections) {
		...
		for (Connection con : connections) {
			...
			if(getHost().getEG().contains(msg.getTo())){//源节点和目的节点在一个社交群
				//Myroutingspray othRouter = (Myroutingspray)other.getRouter();
				MyRouter othRouter = (MyRouter)other.getRouter();
				//目的节点和相遇节点也在一个社区，若相遇节点到目的节点的可能性越大，则转发
				//if(other.getEG().contains(msg.getTo())&&(othRouter.getPredFor(msg.getTo()) > ((Myroutingspray) getHost().getRouter()).getPredFor(msg.getTo()))){
				if(other.getEG().contains(msg.getTo())&&(othRouter.getPredFor(msg.getTo()) > ((MyRouter) getHost().getRouter()).getPredFor(msg.getTo()))){

				newConnections.add(con);
				}
				
			}
			else{//源节点和目的节点不在一个社交群
				...
				//if(other.getEG().contains(msg.getTo())||d1/(Math.acos(cosa1)*getHost().getSpeed())>d2/(Math.acos(cosa2)*other.getSpeed())){//相遇的节点和目的节点在一个社交群，或者相遇的节点速度更快
