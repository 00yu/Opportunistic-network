...
public class MyRouter extends ActiveRouter {
	...
	public static final String MY_NS = "MyRouter";
	...
	private void checkAck(DTNHost otherHost) {
		...
		MyRouter otherRouter = (MyRouter) mRouter;
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		Map<DTNHost, Double> othersPreds = 
			((MyRouter)otherRouter).getDeliveryPreds();
	...
	public void update() {
		...
		if(copiesOne.size() > 0){//自写代码	
			...
			/*for (Message m : copiesOne) {	
				if( getPredFor(m.getTo())<0.8){
					m.updateProperty(MSG_COUNT_PROPERTY,(int)Math.ceil(32*(1-getPredFor(m.getTo()))));
				}
			}*/
		}
		...
		/*List<Message> copiesOneForward = sortByQueueMode(getMessagesWithCopiesOneForward());//喷射阶段
       ...
       for (Connection con : getConnections()) {
    	 //获取相遇节点（此条连接上的邻居节点）
			...
			//第二层循环：副本数为1的所有消息								
			for (Message m : copiesOneForward) {
				//相遇节点有该消息，退出当前循环
				if (othRouter.hasMessage(m.getId())) {
			       ...
			       double P2=othRouter.getPredFor(m.getTo());
			
			  //  if(Ub1-Ua1>=0.35){
			       if((P2-P1)/(P2+P1)>0.1){
					messages.add(new Tuple<Message, Connection>(m,con));//所以把m在con上发送是可行的
				
			      }
			}
				
         }
            //messages有值，即将消息传输给相遇节点							
			if (messages.size() != 0) {
			Collections.sort(messages, new TupleComparator());	
					for (Tuple<Message, Connection> t : messages) {
						Message m = t.getKey();
						Connection con2 = t.getValue();
						if (startTransfer(m, con2) == RCV_OK) {
							return;
						}
					}						
					return;
	...
	public int compare(Tuple<Message, Connection> tuple1,
		...
		double p1 = ((MyRouter)tuple1.getValue().
				getOtherNode(getHost()).getRouter()).getPredFor(
				tuple1.getKey().getTo());
		// -"- tuple2...
		double p2 = ((MyRouter)tuple2.getValue().
				getOtherNode(getHost()).getRouter()).getPredFor(
				tuple2.getKey().getTo());
	...
	{//根据效用复制
		...
		for (Message m : messages) {
			...
			if(copies>=1){	//只向更好的邻居节点喷发消息						
					if(((MyRouter) otherNode.getRouter()).getPredFor(m.getTo()) > getPredFor(m.getTo())){
