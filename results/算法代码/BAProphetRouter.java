 ...
 */
public class BAProphetRouter extends ActiveRouter {
	 ...
	 */
	public BAProphetRouter(Settings s) {
		super(s);
	 ...
	 */
	protected BAProphetRouter(BAProphetRouter r) {
		super(r);
	...
	private void updateDeliveryPredFor(DTNHost host) {
		...
		double newValue = oldValue + (1 - oldValue) * (getDurationFor(host)/(SimClock.getTime()));
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		assert otherRouter instanceof BAProphetRouter : "PRoPHET only works " + 
			" with other routers of same type";
		
		double pForHost = getPredFor(host); // P(a,b)
		Map<DTNHost, Double> othersPreds = 
			((BAProphetRouter)otherRouter).getDeliveryPreds();
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		for (Connection con : getConnections()) {
			...
			BAProphetRouter othRouter = (BAProphetRouter)other.getRouter();
			...
			for (Message m : msgCollection) {
				...
				if ((othRouter.getPredFor(m.getTo())*(100-other.getBufferOccupancy())/100) > getPredFor(m.getTo())) {
	...
	private class TupleComparator implements Comparator 
		...
		public int compare(Tuple<Message, Connection> tuple1,
			...
			double p1 = ((BAProphetRouter)tuple1.getValue().
					getOtherNode(getHost()).getRouter()).getPredFor(
					tuple1.getKey().getTo());
			// -"- tuple2...
			double p2 = ((BAProphetRouter)tuple2.getValue().
					getOtherNode(getHost()).getRouter()).getPredFor(
					tuple2.getKey().getTo());
	...
	public MessageRouter replicate() {
		BAProphetRouter r = new BAProphetRouter(this);
		return r;
	}
