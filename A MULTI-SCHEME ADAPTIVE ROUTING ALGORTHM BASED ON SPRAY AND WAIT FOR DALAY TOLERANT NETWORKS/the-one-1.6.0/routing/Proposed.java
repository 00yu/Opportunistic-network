...
public class Proposed extends ActiveRouter {
	...
	protected Message tryAllMessages(Connection con, List<Message> messages) {
		for (Message m : messages) {
			...
			double similar=getHost().getSimilar(other)/Math.max(getHost().getE().size(), other.getE().size());
			double u1=p1+C*(1-similar);
			double u2=p2+C*(1-similar);
			Integer nrofCopies = (Integer)m.getProperty(MSG_COUNT_PROPERTY);
			if((int)Math.floor(u2/(u1+u2)*nrofCopies)>0 && u1<u2) {
				int retVal = startTransfer(m, con); 
				m.updateProperty(UTILITY, u2/(u1+u2));
				if (retVal == RCV_OK) {
